...
(function ($) {
	$.fn.square1 = function (options) {
		...
		if (_this.settings["keyboard"] && !$this.attr("tabindex"))
		...
		$this
			...
			.children()
			.wrap('<div class="image_wrapper" />'); // Surround all direct decendents with <div> (so that this can work with images or other elements, such as <a> or <ul>)

		$(".image_wrapper", $this).each(function () {
			$img = $(this).find("img");
			$(this)
				.attr("scale-from", $img.attr("scale-from"))
		...
		if ($($this).find(".image_wrapper").length > 1)
			$($this).append(
				'<div class="square1_controls"><span class="square1_prev_image">Previous</span><span class="square1_next_image">Next</span><div class="square1_dots"></div></div>'
			);
		...
		var x = 0;
		$(".image_wrapper", $this).each(function () {
			$(".square1_dots", $this).append(
				'<span data-image-num="' + x + '"></span>'
			);
			x++;
		});
		...
		if (_this.settings["animation"] == "slide") {
			...
			$(".image_wrapper, .wrap_placeholder", $this).each(function () {
				$(this).css({
					left: x * 100 + "%",
					transitionDuration: _this.settings["transition_time"] + "ms",
				});
				x++;
			});
		...
		$($this).on("mouseleave", function () {
			if (_this.settings["pause_on_hover"] && _this.settings["auto_start"])
				run_slideshow();
		});

		// Start slideshow (with pause at the beginning)
		if (_this.settings["auto_start"])
			setTimeout(run_slideshow, _this.settings["start_delay"]);
		update_caption();
		...
		function loadImage($wrapper) {
			...
			if (
				$($this).find(
					".image_wrapper:not(.image_loaded), .wrap_placeholder:not(.image_loaded)"
				).length < 1
			)
				return;
			...
			img.onload = function () {
				...
				if (_this.settings["lazy_load"]) do_nothing = 1;
					...
					load_image_timeout = setTimeout(function () {
						loadImage(
							$wrapper.closest(".square1").find(".image_wrapper").first()
						);
					}, 1000);
				}
				...
				if (
					$wrapper.closest(".square1").find(".image_wrapper:not(.image_loaded)")
						.length < 1
				) {
			...
			$('.wrap_placeholder[background-image="' + img_url + '"]', $this).css(
				"background-image",
				"url(" + img_url + ")"
			);
		}
		...
		function reset_interval() {
			...
			clearInterval(_this.square1_interval);
			clearInterval(_this.square1_interval);
			_this.square1_interval = setInterval(
				next_image,
				_this.settings["slide_duration"] + _this.settings["transition_time"]
			);
		...
		function next_image() {
			...
			var next_slide_index = $(".current_slide", $this)
				.next(".image_wrapper")
				.index();
		...
		function prev_image() {
			var curr_slide_index = $(".current_slide", $this).index();
			var prev_slide_index = $(".current_slide", $this)
				.prev(".image_wrapper")
				.index();
		...
		function jump_to_image(image_num) {
			...
			if (_this.settings["animation"] == "slide") {
				...
				$(".image_wrapper, .wrap_placeholder", $this).removeClass(
					"no_transition"
				);
				...
				if (loop_slider && image_num == 0) {
					...
					if (!$(".wrap_placeholder", $this).first().hasClass("image_loaded"))
						loadImage($(".wrap_placeholder", $this).first());
				...
				$(".image_wrapper, .wrap_placeholder", $this).css(
					"transform",
					"translateX(" + translate_target + "%)"
				);

				if (reset_slide_position == "first") {
					_this.reset_position_timeout = setTimeout(function () {
						$(".image_wrapper, .wrap_placeholder", $this).addClass(
							"no_transition"
						);
						$(".image_wrapper, .wrap_placeholder", $this).css(
							"transform",
			...
			// Fade animation
				...
				if (_this.settings["fill_mode"] == "cover") {
					...
					// Otherwise, show new image and fade out current
						...
						$(".current_slide", $this)
							...
							.fadeOut(_this.settings["transition_time"]);
						$(".image_wrapper", $this)
							.eq(image_num)
							.addClass("current_slide")
							.show();
				...
				// Whereas 'contain' mode looks weird when different sized images stay on the screen too long
					...
					$(".image_wrapper", $this)
						.eq(image_num)
						.addClass("current_slide")
						.fadeIn(_this.settings["transition_time"]);
				}
			}
		...
		function update_dots_nav() {
			...
			$(
				'.square1_dots span[data-image-num="' +
